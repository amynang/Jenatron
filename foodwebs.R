...
# nem.mes.mac.taxonomy = read.csv("taxonomy_traits.csv", row.names = 1, header = TRUE)
...
for (i in 1:length(att)) {
  ...
  fauna = intersect(colnames(web[[i]]), c(nematodes,meso,macro))
  ...
  meanmass = att[[i]]$Bodymass.mg[att[[i]]$taxon_name %in% fauna]
  ...
  props = bodymat*
    att[[i]]$Agility[att[[i]]$taxon_name %in% fauna]*
    att[[i]]$PhysicalProtection[att[[i]]$taxon_name %in% fauna]*
    att[[i]]$Metabolites[att[[i]]$taxon_name %in% fauna]*
    vertical
  ...
  rel.biomass = as.vector(vegan::decostand(att[[i]]$Biomass.mg[att[[i]]$taxon_name %in% fauna],"total", 2))
...
# > c(.1,.3,.6)*c(1/3,1/3,1/3)
# [1] 0.03333333 0.10000000 0.20000000
# > (c(.1,.3,.6)*c(1/3,1/3,1/3))/sum(0.03333333,0.10000000,0.20000000)
# [1] 0.1 0.3 0.6
# > c(.1,.3)*c(1/3,1/3)
# [1] 0.03333333 0.10000000
# > (c(.1,.3)*c(1/3,1/3))/sum(0.03333333,0.10000000)
# [1] 0.25 0.75
